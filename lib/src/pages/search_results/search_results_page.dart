...
import 'package:jappeos_software_center/src/providers/search_provider.dart';
...
class _SearchResultsPageState extends State<SearchResultsPage> {
  ...
  void _increaseVisibleCount() {
    ...
    if (_visibleCount >= provider.searchResult.length) {
      return;
    }
    ...
    setState(() {
      _visibleCount = math.min(provider.searchResult.length, _visibleCount + step);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final apps = provider.searchResult;
    ...
    if (provider.isLoading && apps.isEmpty) {
      ...
      rows.add(
        SectionHeader(
          title: 'Found ${apps.length} apps matching "${provider.searchQuery}"',
